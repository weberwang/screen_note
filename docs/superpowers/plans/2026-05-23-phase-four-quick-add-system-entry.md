...
- 设计源码先行：任何快速添加页、底部弹层、失败兜底页、系统入口说明或原生入口展示实现前，必须先由设计子代理在 `designs/screen_note_stage4.pen` 完成对应节点和状态稿，并同步 `docs/screen-note-phase4-pencil-mapping-2026-05-23.md`。
...
- `Task 3` 拥有快速添加与系统入口说明 `.pen` 设计源码；它必须先于对应 Flutter 页面、弹层和原生入口展示实现完成，缺失视觉状态先补设计映射。
...
- P0 合流后，流程编排、页面和原生桥接才能消费草稿契约与设计源码；不得在页面或 Swift 中新增临时来源字段，也不得用代码临时补视觉状态。
