...
- 已新增 `/quick-add` 路由，并让主 App 在冷启动与恢复前台时都优先消费系统入口写入的待处理草稿；桥接失败会安全降级，不阻断首页主链路。
- 已补齐 Android 原生快速入口中转页、静态快捷方式与共享草稿写入，统一复用 `screen_note.quick_add.intent_payload` 协议回流到 Flutter `/quick-add`。
- 已把 iOS `QuickAddIntent` extension target 接入 `project.pbxproj`，与 Runner/WidgetExtension 共享同一 App Group，准备承接 App Intents 草稿回流。
...
- iOS Quick Add extension 已完成工程 target 接入，但仍需在具备对应签名与 iOS 构建环境后做真机或模拟器侧编译验证。
