...
class AppShellPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final WidgetLaunchBridge launchBridge = ref.watch(
      widgetLaunchBridgeProvider,
    );
    final bool isTaskEditorRoute = currentLocation == RoutePaths.taskEditor;
    // 全局添加入口只服务首页主链路，历史与设置页不承接该入口。
    final bool shouldShowQuickAddButton = currentLocation == RoutePaths.home;
    ...
    return Scaffold(
      ...
      floatingActionButton: isTaskEditorRoute || !shouldShowQuickAddButton
          ? null
          : FloatingActionButton(
              onPressed: () async {
                // 首页浮动入口直接进入正式编辑页，避免壳层继续维护额外的中间弹层状态。
                if (context.mounted) {
