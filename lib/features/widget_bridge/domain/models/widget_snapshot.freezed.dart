...
class _WidgetSnapshot implements WidgetSnapshot {
  const _WidgetSnapshot({required this.snapshotId, required this.generatedAt, required this.displayMode, final  List<WidgetSnapshotItem> items = const <WidgetSnapshotItem>[], this.hasPrivateContent = false, this.hasFallbackContent = false, this.version = 1}): _items = items;
...
@override@JsonKey() List<WidgetSnapshotItem> get items {
  if (_items is EqualUnmodifiableListView) return _items;
  // ignore: implicit_dynamic_type
  return EqualUnmodifiableListView(_items);
}

@override@JsonKey() final  bool hasPrivateContent;
@override@JsonKey() final  bool hasFallbackContent;
